...
import type { KitchenTicket, TicketAge } from '@food/domain';
import { ts } from '../../../tokens/typography';
import styles from './TicketCard.module.css';

export interface TicketCardProps {
  ticket: KitchenTicket;
  /** Mirrors the Figma "Age" property: OnTime / Caution / Late — цвет шапки. */
  age: TicketAge;
 ...
 */
export function TicketCard({ ticket, age, elapsed, hotkey, action }: TicketCardProps) {
  return (
    <article className={[styles.card, styles[age]].join(' ')}>
      <header className={styles.head}>
        ...
        <span className={[styles.elapsed, ts('heading-8/bold')].join(' ')}>{elapsed}</span>
      </header>
