...
/** Округление: «17,6 с.» выглядит как ошибка расчёта, но и грубый шаг на
 *  маленьком счёте превращает 10% в 13% — поэтому шаг зависит от суммы.
 *  Счета в сомони трёхзначные, отсюда мелкие ступени. */
function tipFromPercent(base: number, percent: number): number {
  const exact = (base * percent) / 100;
  const step = exact >= 500 ? 10 : exact >= 100 ? 5 : 1;
