...
**Дизайн-система:** 19 React-компонентов. Ноль хардкода — каждый цвет/отступ/радиус через
CSS-переменную, каждый текст через Text Style. **Есть долг синхронизации с Figma:** `ActionTile`,
`TableCard`, 5 иконок, состояние `Chip/Disabled` и экраны Home/Orders/Bill ещё не зеркалированы —
Figma MCP не был авторизован. Список — в `artifacts/parity-report.md`.
