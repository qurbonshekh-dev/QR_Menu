...
(у пиццы выбор размера и теста) → корзина → оформление → подтверждение заказа. С главной также
доступны «Мои заказы», счёт за стол с чаевыми и вызов официанта с поводом — локальные моки. Данные — локальный мок, бэкенда нет.

**Дизайн-система:** 18 React-компонентов. Ноль хардкода — каждый цвет/отступ/радиус через
...
app/src/
  ...
  pages/               HomePage MenuPage DishPage CartPage CheckoutPage OrderSuccessPage OrdersPage BillPage
